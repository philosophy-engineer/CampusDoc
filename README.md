...
- 현재 코드는 정적 웹 프로토타입 형태이며, `txt` 중심 읽기 인터페이스가 구현되어 있습니다.
...
- 런타임 분리: `electron/main.js`(메인), `electron/preload.js`(IPC 브리지), `index.html + app.js`(렌더러)
...
- `npm run dev`: Electron 실행 + 렌더러 파일(`index.html`, `app.js`) 변경 시 자동 reload
